...
class HashyPerfectionTable(Generic[K, V]):
    ...
    def __init__(self) -> None:
        ...
        self.count: int = 0
        self.valid_keys = [stat.value for stat in PlayerStats]

    def hash(self, key: K) -> int:
        """
        Hash a key for insert/retrieve/update into the hashtable.

        Complexity: 14n + 27, where n is the length of the key
        Best Case Complexity: O(n) since the complexity of ord() is n
        Worst Case Complexity: O(n)
        """
        # Generate a primary hash based on the ASCII sum of the key
        if key not in self.valid_keys:
            raise KeyError(key)

        return (ord(key[0]) * 7 + len(key) * 3 - ord(key[2])) % 13
